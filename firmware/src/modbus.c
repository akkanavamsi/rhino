#include "modbus.h"
#include "communication.h"
#include "anemometer.h"

/* Table of CRC values for high order byte */
const uint8_t auchCRCHi[256] = {

} ;

/* Table of CRC values for low order byte */
const uint8_t auchCRCLo[256] = {

} ;
uint16_t modbusErrorCount = 0;

X2Byte convertorRTU; //Numeric to byte conversion

uint16_t calculateCRC(uint8_t *buff,uint16_t length);
uint16_t getU16(uint8_t buffer[], uint16_t index);
uint32_t getU32(uint8_t buffer[], uint16_t index);
int32_t getI32(uint8_t buffer[], uint16_t index);

//**** Modbus Master ****

uint16_t  creatReadIR(uint8_t buffer[], uint8_t id, uint16_t reg, uint16_t count)
{
    buffer[0] = id;
    buffer[1] = 0x04;
    buffer[2] = (uint8_t)(reg >> 8);
    buffer[3] = (uint8_t)reg;
    buffer[4] = (uint8_t)(count >> 8);
    buffer[5] = (uint8_t)count;
    uint16_t crc = calculateCRC(buffer, 6);
    buffer[6] = (uint8_t)(crc >> 8);
    buffer[7] = (uint8_t)crc;
    return 8;
}

uint16_t  creatReadHR(uint8_t buffer[], uint8_t id, uint16_t reg, uint16_t count)
{
    buffer[0] = id;
    buffer[1] = 0x03;
    buffer[2] = (uint8_t)(reg >> 8);
    buffer[3] = (uint8_t)reg;
    buffer[4] = (uint8_t)(count >> 8);
    buffer[5] = (uint8_t)count;
    uint16_t crc = calculateCRC(buffer, 6);
    buffer[6] = (uint8_t)(crc >> 8);
    buffer[7] = (uint8_t)crc;
    return 8;
}

uint16_t crc;

uint16_t parseFrame(uint8_t buffer[], uint16_t length)
{
    //uint16_t crc = calculateCRC(buffer,length - 2);
    crc = calculateCRC(buffer,length - 2);
    uint16_t crcRec = getU16(buffer, length - 2);
    if(crc == crcRec)
    {
        //Process data and populate struct
        switch (buffer[1])
        {
            case 0x03://Read HR
                break;
            case 0x04://Read IR
                anemometerData.raw.windSpeed = getU32(buffer , INDEX_MEAN_WIND_SPEED + 3);
                anemometerData.raw.windDirection = getU32(buffer , INDEX_MEAN_WIND_DIRECTION + 3);
                anemometerData.raw.internalTemperature = getU32(buffer , INDEX_INTERNAL_TEMP + 3);
                anemometerData.raw.precipitation = getU32(buffer , INDEX_PRECIPITATION + 3);
                anemometerData.raw.compass = getU32(buffer , INDEX_COMPASS + 3);
                anemometerData.raw.radiation = getU32(buffer , INDEX_RADIATION + 3);
                break;
            case 0x06://Single Write
                break;
            case 0x10://Block write
                break;
            default:
                break;
        }
    }
    else
        modbusErrorCount++;
    return 1;
}

//**** Modbus General ****
uint16_t calculateCRC(uint8_t *buff,uint16_t length) //Calculates 16 bit CRC
{
    uint16_t temp = 0xFFFF;
    uint8_t uchCRCHi = 0xFF ; 			/* high byte of CRC initialized */
    uint8_t uchCRCLo = 0xFF ; 			/* low byte of CRC initialized */
    uint8_t uIndex;/* will index into CRC lookup table */
    uint8_t usDataLen;
    uint8_t *puchMsg;
    usDataLen = length;
    puchMsg = buff;
    while (usDataLen) 						/* pass through message buffer */
    {
        uIndex = uchCRCHi ^ *puchMsg++ ; 		/* calculate the CRC */
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex] ;
        uchCRCLo = auchCRCLo[uIndex] ;
        usDataLen--;
    }
    temp = uchCRCLo;
    temp = temp << 8;
    temp = temp | uchCRCHi;
    return SwapBytes(temp);
    //return temp;// Use if CRC bytes dont need swapping
}

uint16_t getU16(uint8_t buffer[], uint16_t index)
{
    convertorRTU.slongValue = 0;
    convertorRTU.byte0 = buffer[index + 1];
    convertorRTU.byte1 = buffer[index];
    return convertorRTU.intValue[0];
}

uint32_t getU32(uint8_t buffer[], uint16_t index)
{
    uint32_t retVal = getU16(buffer, index);
    retVal = (retVal << 16) & 0xFFFF0000;
    retVal |= getU16(buffer, index + 2);
    return retVal;
}

int32_t getI32(uint8_t buffer[], uint16_t index)
{
    int32_t retVal = (int32_t)getU32(buffer, index);
    return retVal;
}

uint16_t getExpectedByteCount(uint8_t buffer[])
{
    uint16_t retVal = 4;
    switch (buffer[1])
    {
        case 0x03:
        case 0x04:
        case 0x17:
            retVal += buffer[2] + 1;
            break;
        case 0x06:
        case 0x10:
            retVal += 4;
            break;
        default:
            retVal = 0;
            break;
    }
    return retVal;
}